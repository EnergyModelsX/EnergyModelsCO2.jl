...
pretty_table(sort(filter(x->x.node.id == "gen", JuMP.Containers.rowtable(
        value,
        m[:flow_out];
...
pretty_table(
    JuMP.Containers.rowtable(
        value,
        m[:emissions_total];
        header = [:tp, :Prod, :emissions_total],
    ),
)
...
# pretty_table(
    # JuMP.Containers.rowtable(
        # value,
        # m[:emissions_strategic];
        # header = [:tinv, :prod, :emissions_strategic],
    # ),
# )
...
pretty_table(
    JuMP.Containers.rowtable(
        value,
        m[:emissions_node];
        header = [:node, :t, :prod, :emissions_node],
    ),
)
